...
% draw(dj.ERD(hillman_mouse_subject.getSchema)) % show the pipeline
describe(hillman_mouse_subject.Strain) % describe the table
%syncDef(hillman_mouse_subject.Strain)    %update a table

% insert entries individually
data = struct('strain_name','cdH5-GCaMP8','strain_description','G8-endo');
insert(hillman_mouse_subject.Strain,data)
...
insert(hillman_mouse_subject.SubjectMark,data)

% see table
hillman_mouse_subject.SubjectMark
...
insert(hillman_mouse_subject.Source,data)

% see table
hillman_mouse_subject.Source
...
insert(hillman_mouse_subject.Allele,data)
...
%% delete strain
del(hillman_mouse_subject.SubjectMark & "mark_type='ear'")
...
% fetch data
fetched_data = fetch(hillman_mouse_subject.Subject,'subject_id');
...
% insert new data
for i = idx_insert
    insert(hillman_mouse_subject.Subject,data(i))
end
